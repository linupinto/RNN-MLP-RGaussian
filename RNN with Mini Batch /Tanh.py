...
base_path=r"C:\Users\HP\Desktop\D folder\RNN\RNN pgm\output_tanh"
...
# -------Parameters------------
layer_dims=[T_feature,64,64,CLS] # input dimension (features per time step),  hidden state size, output dimension (e.g., for classification)
...
def backwardpropagation(X,Y, act, params):
    ...
    for t in range(T, 0, -1):
        if t==T:
            delta[f'd{t}_{L}']=(E @ params['Why']) * tanh_derivative( act[f'Z{t}_{L}'])
        else:
            delta[f'd{t}_{L}']=(delta[f'd{t+1}_{L}']@ params[f'Whh{L}'])* tanh_derivative(act[f'Z{t}_{L}'])

    for l in range(L-1,0,-1):
        for t in range(T,0,-1):            
            delta[f'd{t}_{l}']=(delta[f'd{t+1}_{l}']@ params[f'Whh{l}']+delta[f'd{t}_{l+1}']
                                @  params[f'Wxh{l+1}'])* tanh_derivative(act[f'Z{t}_{l}'])
    # Gradients        
    for l in range(1,L+1):
...
for i in range(1,epoch+1):    
    ...
    for l in range(1, L+2): 
        ...
        if l<L+1:
            ...
            flattened_grads.append(grads['dWhy'].flatten().T) # Flatten the gradient and add it to the list
